...
# operating system:  Linux 4.4.14 x86_64 [ELF] (4.4.14)
# libc:	(6.24.0)
# binutils:	(226)
...
#  -----------------------------------------------------
#  Top Level Imakefile for moxfm
#
#  Created for xfm Mar 8 1993 by Simon Marlow (simonm@dcs.gla.ac.uk)
#  Modified Mar 22 1995 by Albert Graef (ag@muwiinfa.geschichte.uni-mainz.de)
#   Adapted to moxfm Aug 11 1995 by Oliver Mai (mai@x4u2.desy.de)
#  -----------------------------------------------------

#  Global settings for moxfm Imakefiles, to configure
#  moxfm according to your preferences and system.
#
#  Aug 1995 Oliver Mai (last update Jan 1996)

#  Destination directories. *************************

#  Where moxfm config files go.
XFMLIBDIR = $(LIBDIR)/moxfm

#  Where pixmaps and bitmaps go.
#  (You might want to change this to
#  /usr/include/X11/pixmaps etc.)
PIXMAPDIR = $(XFMLIBDIR)/pixmaps
BITMAPDIR = $(XFMLIBDIR)/bitmaps

#  Magic headers. ******************************

#  Comment out the following if you do not want to have
#  the option to use magic headers.

#  Libraries. *************************************

#  Set XMINCDIR and XMLIBDIR as required by
#  your system if the Motif library is not on the standard path.
#  E.g.:
# XMINCDIR = -I/usr/include/Motif1.2
# XMLIBDIR = -L/usr/lib/Motif1.2

#  How to link the Motif library:
XMLIB = -lXm

#  Set XPMINCDIR and XPMLIBDIR as required by
#  your system if libXpm is not on the standard path.
#  E.g.:
...
#  If you have a Linux system and want moxfm to
#  (un)mount filesystems with the usermount utility,
#  uncomment the next line for
#  installing usermount.
# #define MAKE_USMOUNT

#  Compilation options. *****************************

#  Uncomment the following line if you want to compile
#  moxfm with gcc, but gcc is not the default compiler
#  on your system.
# #define USE_GCC
...
#  debug moxfm with Electric fence:
# EXTRA_LIBRARIES = -lefence

#  Uncomment the following two lines if you want to
#  debug moxfm with dmalloc:
# EXTRA_LIBRARIES = -ldmalloc
# DEBUG_DEFINES = -DDMALLOC -DDMALLOC_FUNC_CHECK

#  System dependencies.****************************

#  This must point to the file which contains the
#  table of mounted devices.
#  You might need to set this to /etc/mnttab on some
#  systems (HP-UX, e.g.).
MTABPATH = /etc/mtab

#  This should point to the mail spooling directory.
#  You might need to change it to /usr/spool/mail,
#  /usr/mail (HP-UX) or /var/mail (IRIX)
MAILDIR = /var/spool/mail

#  Uncomment the next two lines if you are running
#  a SVR4 system like e.g. Solaris:
# SVR4_INC = -I/usr/dt/include -I/opt/X11/include
# SVR4_LIBDIR = -L/usr/dt/lib

#  Uncomment the next line if you cannot compile
#  FmMount.c on your system (default on OSF/1).
#  Read the note in INSTALL about this option!
# MOUNT_DEF = -DNO_GETMNTENT

#  Uncomment the next line if you are running IRIX.
# IRIX_DEF = -DIRIX

#  Uncomment the next line if your system is an SGI machine
#  and you want moxfm to have the SGI Motif look.
# #define SGI_MODE

#  Uncomment the next line if you are running OSF/1
#  or Digital Unix
# #define OSF1

#  Uncomment the next line if your system has the
#  statvfs() system call (default if SVR4 is defined).
#  Check this setting if FmDirs.c and FmFw.c won't compile,
#  or when your linker cannot resolve _statfs.
# STATFSTYPE = -DUSE_STATVFS

#  Uncomment the following line if you have the realpath()
#  funktion in your C library (not necessary on Linux and SVR4).
# REALPATH = -DHAVE_REALPATH

#  Uncomment the next line if your C library doesn't contain
#  the memmove() function (as on some SunOS 4.x systems):
# MEMMOVE = -DNO_MEMMOVE
...
#  rules to create and install the appdefaults file, the
#  config files and the
#  pixmaps

#

Moxfm.ad::  lib/Moxfm.cpp
	$(RM) $@
	 	$(CPP)  -DPIXMAPDIR=$(PIXMAPDIR) -DBITMAPDIR=$(BITMAPDIR) -DXFMRC=$(XFMRC) <lib/Moxfm.cpp | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@
...
install:: Moxfm.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) Moxfm.ad $(DESTDIR)$(XAPPLOADDIR)/Moxfm
...
install::
	@if [ -d $(XFMLIBDIR) ]; then \
		...
		if [ -h $(XFMLIBDIR) ]; then \
			(set -x; rm -f $(XFMLIBDIR)); \
		fi; \
		(set -x; $(MKDIRHIER) $(XFMLIBDIR)); \
	fi
	$(INSTALL) -c $(INSTBINFLAGS) lib/checkmail $(XFMLIBDIR)/checkmail
	@if [ -d $(XFMLIBDIR)/dot.moxfm ]; then \
		set +x; \
	else \
		if [ -h $(XFMLIBDIR)/dot.moxfm ]; then \
			(set -x; rm -f $(XFMLIBDIR)/dot.moxfm); \
		fi; \
		(set -x; $(MKDIRHIER) $(XFMLIBDIR)/dot.moxfm); \
	fi
	...
	@if [ -d $(PIXMAPDIR) ]; then \
		...
		if [ -h $(PIXMAPDIR) ]; then \
			(set -x; rm -f $(PIXMAPDIR)); \
		fi; \
		(set -x; $(MKDIRHIER) $(PIXMAPDIR)); \
	fi
	$(INSTALL) -c $(INSTDATFLAGS) lib/pixmaps/?* $(PIXMAPDIR)

#  Use the following before installing over an existing
#  version of moxfm. (You might wish to back up
#  your old configuration first. ;-)
